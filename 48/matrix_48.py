...
def generate_dataset(num_samples=50000):
    """生成48×48 0-1矩阵及其可逆性标签"""
    print(f"正在生成 {num_samples} 个48×48矩阵...")
    start_time = time.time()

    matrices = np.random.randint(0, 2, size=(num_samples, 48, 48))
    ...
    for i in range(0, num_samples, batch_size):
        ...
        labels[i:end_idx] = (np.array(ranks) == 48).astype(np.int32)
...
def enhanced_augment_data(matrices, labels):
    ...
    # 全零行/列
    for _ in range(500):
        mat = np.random.randint(0, 2, (48, 48))
        if np.random.rand() > 0.5:
            mat[np.random.randint(0, 48)] = 0  # 全零行
        else:
            mat[:, np.random.randint(0, 48)] = 0  # 全零列
    ...
    for _ in range(500):
        mat = np.random.randint(0, 2, (48, 48))
        if np.random.rand() > 0.5:
            row1, row2 = np.random.choice(48, 2, replace=False)
            mat[row2] = mat[row1]  # 相同行
        else:
            col1, col2 = np.random.choice(48, 2, replace=False)
    ...
    for _ in range(500):
        ...
        if np.random.rand() > 0.5:
            row1, row2 = np.random.choice(48, 2, replace=False)
            mat[row2] = (mat[row1] + mat[np.random.choice(48)]) % 2  # 行线性相关
        else:
            col1, col2 = np.random.choice(48, 2, replace=False)
            mat[:, col2] = (mat[:, col1] + mat[:, np.random.choice(48)]) % 2  # 列线性相关
        special_non_invertible.append(mat)
...
def main():
    ...
    # 重塑矩阵为8×8×1
    matrices = matrices.reshape((-1, 48, 48, 1))
    ...
    test_cases = [
        ...
        # 不可逆矩阵示例
        {
            "matrix": np.ones((48, 48)),
            "name": "全1矩阵"
        },
    ]

    for case in test_cases:
        input_data = case["matrix"].reshape(1, 48, 48, 1).astype(np.float32)
