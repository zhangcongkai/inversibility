...
def gf2_rank_bitpack(matrix):
    """
    计算64×64二进制矩阵在GF(2)上的秩 using bit packing.
    """
    n = matrix.shape[0]
    # 将每一行转换为一个64位整数：第0列对应整数的最高位（第63位），第63列对应整数的最低位（第0位）
    rows = []
    for i in range(n):
        row_val = 0
        for j in range(n):
...
def generate_dataset(num_samples=100000):
    """生成64×64 0-1矩阵及其可逆性标签"""
    print(f"正在生成 {num_samples} 个64×64矩阵...")
    start_time = time.time()

    matrices = np.random.randint(0, 2, size=(num_samples, 64, 64))
    ...
    for i in range(0, num_samples, batch_size):
        ...
        # 对于每个矩阵，计算GF(2)秩，如果秩为64则可逆
        ranks = [gf2_rank_bitpack(mat) for mat in batch]
        labels[i:end_idx] = (np.array(ranks) == 64).astype(np.int32)
...
def enhanced_augment_data(matrices, labels):
    ...
    # 全零行/列
    for _ in range(500):
        mat = np.random.randint(0, 2, (64, 64))
        if np.random.rand() > 0.5:
            mat[np.random.randint(0, 64)] = 0  # 全零行
        else:
            mat[:, np.random.randint(0, 64)] = 0  # 全零列
    ...
    for _ in range(500):
        mat = np.random.randint(0, 2, (64, 64))
        if np.random.rand() > 0.5:
            row1, row2 = np.random.choice(64, 2, replace=False)
            mat[row2] = mat[row1]  # 相同行
        else:
            col1, col2 = np.random.choice(64, 2, replace=False)
    ...
    for _ in range(500):
        ...
        if np.random.rand() > 0.5:
            row1, row2 = np.random.choice(64, 2, replace=False)
            mat[row2] = (mat[row1] + mat[np.random.choice(64)]) % 2  # 行线性相关
        else:
            col1, col2 = np.random.choice(64, 2, replace=False)
            mat[:, col2] = (mat[:, col1] + mat[:, np.random.choice(64)]) % 2  # 列线性相关
        special_non_invertible.append(mat)
...
def plot_training_history(history):
    ...
    # 精度-召回曲线
    plt.subplot(2, 3, 3)
    plt.plot(history.history['precision'], 'g-', label='训练精度')
    plt.plot(history.history['val_precision'], 'm-', label='验证精度')
    plt.plot(history.history['recall'], 'c-', label='训练召回率')
    plt.plot(history.history['val_recall'], 'y-', label='验证召回率')
    plt.title('精度与召回率', fontsize=14)
    ...
    # AUC曲线
    plt.subplot(2, 3, 4)
    plt.plot(history.history['auc'], 'b-', label='训练AUC')
    plt.plot(history.history['val_auc'], 'r-', label='验证AUC')
    plt.title('AUC曲线', fontsize=14)
    ...
    # PR曲线
    plt.subplot(2, 3, 5)
    plt.plot(history.history['prc'], 'b-', label='训练PRC')
    plt.plot(history.history['val_prc'], 'r-', label='验证PRC')
...
def main():
    # 生成数据集
    matrices, labels = generate_dataset(num_samples=20000)
    ...
    # 重塑矩阵为8×8×1
    matrices = matrices.reshape((-1, 64, 64, 1))
    ...
    for case in test_cases:
        input_data = case["matrix"].reshape(1, 64, 64, 1).astype(np.float32)
